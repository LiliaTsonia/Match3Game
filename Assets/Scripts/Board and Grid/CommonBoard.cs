...
public class CommonBoard : MonoBehaviour, ICommonBoard {
	...
	void Start()
	{
		...
		_tileOffset = _tilePrefab.GetComponent<SpriteRenderer>().bounds.size;
    ...
    {
		var tileObject = Instantiate(_tilePrefab, new Vector3(_boardPosition.x + (_tileOffset.x * xPos), _boardPosition.y + (_tileOffset.y * yPos), 0), _tilePrefab.transform.rotation, transform);
